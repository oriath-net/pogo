...
* **Releases** is the range of major releases that data files were included
  in. "All" indicates that the file has been observed in all known releases.

* **Structure** is how much work has been done to determine the structure of
  this data file:

  * "no" means that the file's structure is entirely unknown.

  * "partial" means that the file's structure is partially known.

  * "yes" means that the file's structure is largely understood.

  * "empty" means that the file contains no data -- it has either no rows or
    no columns.
